...
class SimplifiedBaggingRegressor:
    ...
    def _generate_splits(self, data: np.ndarray):
        ...
        for bag in range(self.num_bags):
            ...
            self.indices_list.append(
                np.random.randint(0, data_length, data_length))
    ...
    def fit(self, model_constructor, data, target):
        ...
        for bag in range(self.num_bags):
            ...
            data_bag, target_bag = data[self.indices_list[bag]
                                        ], target[self.indices_list[bag]]  # Your Code Here
    ...
    def _get_oob_predictions_from_every_model(self):
        ...
        # Your Code Here
        [list_of_predictions_lists[i].append(*self.models_list[j].predict(self.data[i].reshape(
            1, -1))) if i not in self.indices_list[j] else 0 for j in range(len(self.indices_list)) for i in range(len(self.data))]
    ...
    def _get_averaged_oob_predictions(self):
        ...
        self.oob_predictions = [np.mean(el) if len(
            el) > 0 else None for el in self.list_of_predictions_lists]  # Your Code Here
